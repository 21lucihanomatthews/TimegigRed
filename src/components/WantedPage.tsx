...
export default function WantedPage({ onToggleForm, onToggleView, onChatRequest }: Props) {
  const { profile } = useUser();
  const { wantedItems: items, addWantedItem, updateWantedItem, deleteWantedItem } = useData();
    ...
    <div className="space-y-4">
      {congratulate && (
        <div className="p-4 bg-green-100 text-green-700 rounded-lg text-center font-bold">
          Congratulations! Your request is now live in the market!
        </div>
      )}
      {deletedMsg && (
      ...
      {!showForm ? (
        ...
        <ItemForm type="wanted" onPost={handlePost} onCancel={() => toggleForm(false)} initialData={editingItem || undefined} />
      )}
    </div>
  );
}
