...
interface Props {
  type: 'sell' | 'wanted';
  onPost: (item: MarketItem) => void;
  onCancel: () => void;
  initialData?: MarketItem;
}

export default function ItemForm({ type, onPost, onCancel, initialData }: Props) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-xl space-y-4 relative z-[60]">
      <div className="flex items-center gap-4 mb-2">
        ...
        <h2 className="text-xl font-black italic tracking-tighter uppercase">{initialData ? 'Edit Item' : (type === 'sell' ? 'Sell an Item' : 'Looking for an Item')}</h2>
      </div>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1">
          <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 px-1">Title</label>
          <input type="text" placeholder="Item name..." className="w-full h-12 px-4 border border-zinc-100 rounded-xl bg-zinc-50 focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all outline-none" value={title} onChange={(e) => setTitle(e.target.value)} required />
        </div>

        <div className="space-y-1">
          <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 px-1">Description</label>
          <textarea placeholder="Describe the item..." className="w-full p-4 border border-zinc-100 rounded-xl bg-zinc-50 focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all outline-none min-h-[120px]" value={description} onChange={(e) => setDescription(e.target.value)} required />
        </div>
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 px-1">{type === 'sell' ? 'Price' : 'Target Price'}</label>
          <input type="text" placeholder="e.g. R500" className="w-full h-12 px-4 border border-zinc-100 rounded-xl bg-zinc-50 focus:bg-white transition-all outline-none" value={price} onChange={(e) => setPrice(e.target.value)} required />
        </div>
        
        <div className="space-y-3 pt-2">
          <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 px-1 block">Item Images</label>
